...
class Mod:
		...
		async def role_add(self, ctx, user: discord.Member, role: discord.Role):
			"""Add the role to the user. Case-sensitive.

				Examples:
				[p]role add Iwatalemon Muted
				[p]role add DSX8 "Helpers" """
			author = ctx.message.author
			server = author.server

			if author == user:
				await self.bot.say("I won't let you set your own role, you baka!")
				return

			if not self.is_allowed_by_hierarchy(server, author, user):
				await self.bot.say("You're not higher than the user in the role hierarchy. Therefore, I cannot grant your request. Senpai said so...")
				return

			try:
				if role in user.roles:
					await self.bot.say("Muda da. This role is already set on **{}**.".format(user))
					return
				await self.bot.add_roles(user, role)
				await self.bot.say("The role '{}' has been added to **{}** correctly.".format(role, user))
			except discord.errors.Forbidden:
				await self.bot.say("I cannot do that.")
			except:
				await self.bot.say("No.")

		@_role.command(name="remove", pass_context=True)
		async def role_remove(self, ctx, user: discord.Member, role: discord.Role):
				"""Remove the user role. Case-sensitive.

				Examples:
				[p]removerole Nano Researchers 
				[p]removerole UltraMario "Mini Mods" """
				author = ctx.message.author
				server = author.server
				...
				try:
						...
						if role not in user.roles:
								await self.bot.say("The role '{}' is not set on **{}**".format(role, user))
								return

						await self.bot.remove_roles(user, role)
						await self.bot.say("The role '{}' has been removed from **{}** successfully. Can I get a pat on the head? :relieved:".format(role, user))
				except discord.errors.Forbidden:
						await self.bot.say("I cannot do that.")
				except:
						await self.bot.say("No.")
		...
		async def ban(self, ctx, user: discord.Member, days: str = None, *, reason: str = None):
				...
				try:
						...
						await self.new_case(server,
																...
																user=user,
																reason=reason)
						await self.bot.say("Done. I-It's not like I want to or anything, but Senpai said so...")
						await self.bot.say("https://cdn.discordapp.com/attachments/232412312827461633/381794207578456064/nocontxtmario-DO8kuXYXkAElNKh.gif")
				except discord.errors.Forbidden:
						await self.bot.say("I'm not allowed to do that.")
				except Exception as e:
						print(e)
		...
		async def messages(self, ctx, number: int):
				...
				async for message in self.bot.logs_from(channel, limit=number+1):
						if message.id == "377139833807831041":
							continue
						to_delete.append(message)
